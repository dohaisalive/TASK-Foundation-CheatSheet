...
- `{ }`: is mainly used for `containing classes and methods`
- `( )`: is mainly used for `contaning a function's parameters and arguments`
- `[ ]`: is mainly used for `contaning an array`
...
if(x!=5){
  ...
  // do job 2
  // This code will only be executed if the condition inside the if didn't work
...
function foo(functionX,functionY){
}
...
console.log(array.length); // prints the number of elements in the array
//
```
...
```js
array.push(x); // adds a new element
...
- Iteration methods work on arrays, using arrow functions only!
